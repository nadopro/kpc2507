...
로컬스토리지에 잘 저장되고, 동작도 정상적인 것을 확인했어.
그런데, 비밀번호의 raw data가 그대로 보이는 문제가 있어서,
localStorage에 저장할 때, 암호화 해 저장하고 싶어.
그런데, 다시 이 페이지에 왔을 때는 암호화된 값을 복호화 해,
CheckBox가 체크된 경우에는 원 텍스트가 입력된 것 처럼 보여주고 싶어.
예를 들어서 base64_encode와 같은 복호화가 가능한 알고리즘으로
암호화 해 저장하고 싶어.
여기까지만 코드를 수정해 줘.

Q 18.
잘 동작하는 것을 확인했어.
이제 Burp Suite와 같은 웹 프락시로 검출하면 
pass가 노출되는 문제를 해결하고 싶어.
submit이 클릭되는 순간, 입력된 pass를 같은 방법으로 암호화해서
네트워크로 보내고 작업을 추가하고 싶어.
같은 방법으로 base64로 변환해 전송하는 코드로 변경해 줘.


Q 19.
BurpSuite에서 암호화되어 전송되는 것을 확인했어.
이제 서버쪽의 로그인 하는 코드는 수신한 암호화된 데이터를 
같은 알고리즘으로 복호화 해 비교하도록 바꾸고 싶어.
원 코드는 다음과 같아.
여기에 수신한 비밀번호를 base64로 복호화해 DB와 비교하도록
코드를 변경해 줘.
